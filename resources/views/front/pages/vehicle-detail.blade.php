                    ...
                    @if($vehicle->loueur)
                        <div class="space-y-3">
                            @if($vehicle->loueur->phone)
                                <a href="tel:{{ $vehicle->loueur->phone }}"
                                   data-track="phone"
                                   data-vehicle-id="{{ $vehicle->id }}"
                                   data-loueur-id="{{ $vehicle->loueur->id }}"
                                   data-phone="{{ $vehicle->loueur->phone }}"
                                   class="flex items-center justify-center gap-2 w-full py-3 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-700 transition shadow-lg shadow-amber-600/20">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                                    Appeler
                                </a>
                            @endif
                            @if($vehicle->loueur->whatsapp)
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $vehicle->loueur->whatsapp) }}?text={{ urlencode('Bonjour, je suis intéressé par ' . $vehicle->full_name . ' sur ResaDZ.') }}"
                                   target="_blank"
                                   data-track="whatsapp"
                                   data-vehicle-id="{{ $vehicle->id }}"
                                   data-loueur-id="{{ $vehicle->loueur->id }}"
                                   data-phone="{{ $vehicle->loueur->whatsapp }}"
