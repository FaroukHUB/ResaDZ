        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-24">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-3">
                    @if($logoLight)
                        <img src="{{ Storage::url($logoLight) }}" alt="{{ $siteName }}" class="h-20 w-auto">
                    @else
                        {{-- Fallback text logo --}}
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
                    ...
                    <div class="lg:col-span-4">
                        <div class="flex items-center gap-3 mb-6">
                            @if($logoDark)
                                <img src="{{ Storage::url($logoDark) }}" alt="{{ $siteName }}" class="h-24 w-auto">
                            @elseif($logoLight)
                                <img src="{{ Storage::url($logoLight) }}" alt="{{ $siteName }}" class="h-24 w-auto brightness-0 invert">
                            @else
                                <span class="text-3xl font-black text-white">{{ $siteName }}</span>
                            @endif
                        </div>
                        <p class="text-white/50 leading-relaxed mb-8">La marketplace de location de voitures en Algérie. Trouvez le véhicule idéal auprès de loueurs professionnels vérifiés.</p>

                        {{-- Social Links --}}
                        <div class="flex items-center gap-3">
                            @if($socialFacebook)
                            <a href="{{ $socialFacebook }}" target="_blank" rel="noopener" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-blue-600 text-white/50 hover:text-white transition border border-white/10">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.77 7.46H14.5v-1.9c0-.9.6-1.1 1-1.1h3V.5h-4.33C10.24.5 9.5 3.44 9.5 5.32v2.15h-3v4h3v12h5v-12h3.85l.42-4z"/></svg>
                            </a>
                            @endif
                            @if($socialInstagram)
                            <a href="{{ $socialInstagram }}" target="_blank" rel="noopener" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-gradient-to-br hover:from-purple-600 hover:to-pink-500 text-white/50 hover:text-white transition border border-white/10">
                            ...
                            @endif
                            @if($socialWhatsapp)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $socialWhatsapp) }}" target="_blank" rel="noopener" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-green-600 text-white/50 hover:text-white transition border border-white/10">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <ul class="space-y-4">
                            <li><a href="{{ route('home') }}" class="text-white/50 hover:text-green-400 transition flex items-center gap-2 group"><span class="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-green-400 transition"></span>Accueil</a></li>
                            <li><a href="{{ route('vehicles.index') }}" class="text-white/50 hover:text-green-400 transition flex items-center gap-2 group"><span class="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-green-400 transition"></span>Véhicules</a></li>
                            <li><a href="{{ route('blog.index') }}" class="text-white/50 hover:text-green-400 transition flex items-center gap-2 group"><span class="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-green-400 transition"></span>Actualités</a></li>
                        </ul>
                    </div>
            ...
            <div class="border-t border-white/10 py-6">
                <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                    <p class="text-white/30 text-sm">&copy; {{ date('Y') }} {{ $siteName }}. Tous droits réservés.</p>
                    <div class="flex items-center gap-6 text-white/30 text-sm">
                        <a href="#" class="hover:text-white/60 transition">Mentions légales</a>
                        <a href="#" class="hover:text-white/60 transition">CGU</a>
                        <a href="#" class="hover:text-white/60 transition">Confidentialité</a>
                    </div>
                </div>
            </div>
        </div>
