<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div style="display: flex; gap: 8px; background: #f1f5f9; padding: 6px; border-radius: 14px; width: fit-content;">
            ...
            <button wire:click="$set('activeTab', 'clients')"
                style="padding: 10px 20px; border-radius: 10px; font-weight: 600; font-size: 14px; border: none; cursor: pointer; transition: all 0.2s;
                {{ $activeTab === 'clients' ? 'background: linear-gradient(135deg, #059669, #10B981); color: white; box-shadow: 0 4px 12px rgba(16,185,129,0.3);' : 'background: transparent; color: #64748b;' }}">
                Clients
            </button>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            {{-- Liste des modèles clients --}}
            <div class="lg:col-span-2 space-y-3">
                <h3 style="font-size: 13px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px;">Message au client</h3>
                @php
                    $clientIcons = [
                        'avis_google_client' => '⭐',
                        'suivi_reservation' => '🚗',
                ...
                @endphp
                @foreach($clientTemplates as $key => $template)
                    <div wire:click="$set('clientSelectedTemplate', '{{ $key }}')"
                         style="padding: 14px 16px; border-radius: 12px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 12px;
                         {{ $clientSelectedTemplate === $key ? 'background: linear-gradient(135deg, #ECFDF5, #D1FAE5); border: 2px solid #10B981; box-shadow: 0 2px 8px rgba(16,185,129,0.15);' : 'background: white; border: 1px solid #e2e8f0;' }}">
                        <span style="font-size: 22px;">{{ $clientIcons[$key] ?? '📧' }}</span>
                        <span style="font-weight: 600; font-size: 14px; color: #1e293b;">{{ $template['label'] }}</span>
                    </div>
                @endforeach
            </div>

            {{-- Destinataire + aperçu + envoi --}}
            <div class="lg:col-span-3 space-y-4">

                <div style="background: white; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px;">
                    <h3 style="font-size: 13px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 12px;">Client destinataire</h3>
                    <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                        <button wire:click="$set('clientRecipientType', 'client')"
                            style="padding: 8px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; border: none;
                            {{ $clientRecipientType === 'client' ? 'background: #065F46; color: white;' : 'background: #f1f5f9; color: #64748b;' }}">
                            Un client
                        </button>
                        <button wire:click="$set('clientRecipientType', 'custom')"
                            style="padding: 8px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; border: none;
                            {{ $clientRecipientType === 'custom' ? 'background: #065F46; color: white;' : 'background: #f1f5f9; color: #64748b;' }}">
                            Contact libre
                        </button>
                    </div>
                    ...
                    @else
                        <div style="display: flex; gap: 8px;">
                            <input type="email" wire:model.live="clientCustomEmail" placeholder="email@exemple.com" style="flex: 1; padding: 10px 14px; border-radius: 10px; border: 1px solid #d1d5db; font-size: 14px;" />
                            <input type="tel" wire:model.live="clientCustomPhone" placeholder="Tél. WhatsApp (ex: 0550...)" style="flex: 1; padding: 10px 14px; border-radius: 10px; border: 1px solid #d1d5db; font-size: 14px;" />
                        </div>
                    @endif
                </div>

                @if($clientPreviewSubject)
                    <div style="background: white; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden;">
                        <div style="background: linear-gradient(135deg, #065F46, #059669); padding: 16px 20px; display: flex; align-items: center; gap: 12px;">
                            ...
                            <div style="flex: 1;">
                                <p style="color: white; font-weight: 700; font-size: 14px; margin: 0;">Message au client</p>
                                <p style="color: rgba(255,255,255,0.6); font-size: 12px; margin: 2px 0 0 0;">Modifiable avant envoi</p>
                            </div>
                        </div>
                        ...
                        <div style="padding: 20px;">
                            <div style="margin-bottom: 16px;">
                                <label style="font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">Sujet (email)</label>
                                <input type="text" wire:model="clientPreviewSubject" style="width: 100%; padding: 10px 14px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 15px; font-weight: 600; margin-top: 4px;" />
                            </div>
                            <div>
                                <label style="font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px;">Corps du message</label>
                                <textarea wire:model.live="clientPreviewBody" rows="12" style="width: 100%; padding: 14px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 14px; line-height: 1.6; margin-top: 4px; resize: vertical;"></textarea>
                            </div>
                        </div>

                        <div style="padding: 0 20px 20px; display: flex; gap: 10px;">
                            <button wire:click="sendClientTemplate"
                                style="flex: 1; padding: 14px; background: linear-gradient(135deg, #22c55e, #16a34a); color: white; font-weight: 700; font-size: 15px; border: none; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 14px rgba(34,197,94,0.3);">
                                <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg>
                                Envoyer l'email
                            </button>
                            @php $waClientUrl = $this->getClientWhatsappUrl(); @endphp
                            @if($waClientUrl)
                ...
                @else
                    <div style="background: white; border: 1px solid #e2e8f0; border-radius: 16px; padding: 60px 20px; text-align: center;">
                        <svg width="64" height="64" fill="none" stroke="#d1d5db" viewBox="0 0 24 24" stroke-width="1" style="margin: 0 auto 16px;"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        <p style="color: #94a3b8; font-size: 15px;">Sélectionnez un message à gauche</p>
                    </div>
                @endif
            </div>
        </div>
