...
class CourseAvailability extends Page implements HasForms
{
    ...
    protected static string $view = 'filament.chauffeur.pages.course-availability';
    ...
    public function quickAddAvailability(string $date): void
    {
        ...
        if ($existing) {
            ...
            CourseAvailabilityModel::create([
                'loueur_id' => $loueur->id,
                'type' => 'available',
                'for_transfer' => $loueur->offers_transfer ?? true,
                'for_delivery' => $loueur->offers_delivery ?? true,
                'date' => $dateObj,
                'recurrence' => 'none',
            ]);
            Notification::make()
